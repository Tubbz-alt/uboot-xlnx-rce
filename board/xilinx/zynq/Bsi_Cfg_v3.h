...
/* ---------------------------------------------------------------------- *//*
  ...
  The Cluster Switch configuration has changed between BSI version 2
  and version 3. Now each switch port is described by 1 4-byte word. 
  This word is 4 single bytes, each of which enumerate the lane setting. 

  To maintain compatibility with the Fulcrum switch, 24-ports will still
  be defined, even though the Mellanox switch will only need to have the
  9 RCE and 13 Fabric ports defined. (The SFP ports are handled directly
  by the switch.)

  In the case of DPMs with more than 2 RCEs (or any where a single quad
  is shared between RCEs) the IPMC will ensure that if the RCEs specify 
  conflicting port definitions, the port will be reported as not connected.
  All other checking for valid configurations will have to be done by the 
  software running on the DTM.  
  ...
  The Status word indicates that the IPMC has completed filling in this
  block. The contents are:

  bit[31]    = Done
  bit[30]    = Error
  bit[29..0] = MBZ

  Where Done indicates that this block has been completely filled, and
  Error indicates that the block will not be filled due to an error. (Like
  the information retrieved from the Shelf FRU Info could not be parsed.
...
*/
enum {BSI_CLUSTER_CFG_VERSION = 3};
...
typedef enum _BSI_COMMON_CFG_OFFSETS
{
   ...
   BSI_CLUSTER_IP_INFO_RESERVED_OFFSET      = 0xC7,
