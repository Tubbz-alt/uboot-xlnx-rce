...
void zynq_slcr_gem_clk_setup(u32 gem_id, u32 rclk, u32 clk)
{
    ...
    /* smaldona@slac.stanford.edu - fix for EMIO */
	if (gem_id) {
		/* Configure GEM_RCLK_CTRL */
        val = readl(&slcr_base->gem1_rclk_ctrl);
        val |= rclk;
		writel(val, &slcr_base->gem1_rclk_ctrl);
	} else {
		/* Configure GEM_RCLK_CTRL */
        val = readl(&slcr_base->gem0_rclk_ctrl);
        val |= rclk;        
		writel(val, &slcr_base->gem0_rclk_ctrl);
	}
	udelay(100000);
out:
	zynq_slcr_lock();
